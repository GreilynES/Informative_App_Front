...
export function NecesidadesSection({ form, showErrors = false }: NecesidadesSectionProps) {
  ...
  useEffect(() => {
    ...
    // Validar cuando cambian las necesidades
    const necesidadesFilled = necesidades.filter((n) => n && n.trim() !== "").length;
    if (showErrors && necesidadesFilled === 0) {
      setError("Debe ingresar al menos una necesidad o mejora");
    } else {
      setError("");
    }
  }, [necesidades, form, showErrors]);
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-[#4A4A4A] mb-3">
            Mencione las necesidades o mejoras que su finca requiere actualmente (hasta 5, al menos 1): *
          </label>
              ...
              <div key={num}>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-[#6F8C1F] focus:border-[#6F8C1F] ${
                    showErrors && num === 1 && !necesidades[0].trim() 
                      ? "border-red-500" 
                      : "border-[#CFCFCF]"
                  }`}
                  maxLength={255}
