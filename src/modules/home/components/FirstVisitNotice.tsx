...
export function FirstVisitNotice({
  ...
  const startKey = `${storageKey}__startAt`

  const markSeen = () => {
    if (!store) return
    try {
      store.setItem(storageKey, "true")
      store.removeItem(startKey)
    } catch {}
  }
  ...
  useEffect(() => {
    ...
    if (!seen) {
      try {
        const existingStart = store.getItem(startKey)
        if (!existingStart) store.setItem(startKey, String(Date.now()))
      } catch {}
  ...
  useEffect(() => {
    ...
    const getStartAt = () => {
      ...
      try {
        const raw = store.getItem(startKey)
    ...
    if (reducedMotion) {
      ...
      closeDelayRef.current = window.setTimeout(() => close(), remaining + closeDelayMs)
      return () => {
        if (closeDelayRef.current) window.clearTimeout(closeDelayRef.current)
        closeDelayRef.current = null
      }
    ...
    timerRef.current = window.setInterval(() => {
      ...
      if (pct >= 100) {
        if (timerRef.current) window.clearInterval(timerRef.current)
        timerRef.current = null
        closeDelayRef.current = window.setTimeout(() => close(), closeDelayMs)
      }
    }, 100)
      ...
      <div className="animate-in fade-in slide-in-from-top-2 duration-300">
        ...
        >
          <ItemMedia
            variant="icon"
            className="
              flex items-center justify-center rounded-lg bg-[#1F3D2B] text-[#FAFDF6] shadow-sm h-9 w-9 sm:h-10 sm:w-10 shrink-0 self-start
            "
          >
          ...
          <ItemContent className="min-w-0">
            ...
            <ItemDescription
              className="
                mt-0.5 text-[#FAFDF4]/85 min-w-0 overflow-hidden [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical]
              "
            >
              {event.description}
            </ItemDescription>
